...
function canvasPlayerHook() {
  let menu = document.getElementsByClassName(
    "css-1v3tle5-player-control-container"
  )[0];
  ...
  element.innerHTML =
    '<img alt="LL" src="https://cabalex.github.io/lecturelighter/logo192.png" style="height: 80%;">';
  element.onclick = () => {
  ...
  fetch("/P/Data/VideoJSON", {
    ...
    .then((json) => {
      if (json.video.videoLinkMp4) {
        ...
        element.innerHTML =
          '<img alt="LL" src="https://cabalex.github.io/lecturelighter/logo192.png" style="width: 32px; cursor: pointer;">';
        element.onclick = () => {
          pauseAllMedia();
          element.style = "height: unset; cursor: wait;";
          chrome.runtime.sendMessage({
            type: "OPEN_VIDEO",
            url: json.video.videoLinkMp4,
            title: json.video.videoTitle,
          });
        };
      ...
      fetch(
        `/P/Data/VideoSource?video=${encodeURIComponent(
          json.video.videoLink
        )}&videoPID=${v}&videoListNodePID=${node}&mp4Only=true&includeThumbnails=false&contentToken=${
          json.contentToken
        }`,
        ...
        .then((newJson) => {
          let streams = [];
          let audio = [];
          for (let i = 0; i < newJson.streams.length; i++) {
            const stream = newJson.streams[i];
            if (stream.isAudioOnly) {
              //
              audio.push(
                Object.values(stream.typeAndVideoSourceMap)[0].fileURL
              );
            } else if (stream.typeAndVideoSourceMap.MP4) {
              streams.push(stream.typeAndVideoSourceMap.MP4.fileURL);
            } else if (stream.typeAndVideoSourceMap.AUDIO) {
              audio.push(stream.typeAndVideoSourceMap.AUDIO.fileURL);
            }
          }
          console.log("Found streams and audio", streams, audio);
          ...
          for (let i = 0; i < streams.length; i++) {
            ...
            element.innerHTML = `<img alt="LL" src="https://cabalex.github.io/lecturelighter/logo192.png" style="width: 32px; cursor: pointer;"><span style="position: absolute; right: 5px; bottom: 0; color: white">${
              i + 1
            }</span>`;
            element.onclick = () => {
              pauseAllMedia();
...
function zoomPlayerHook() {
  ...
  element.innerHTML =
    '<img alt="LL" src="https://cabalex.github.io/lecturelighter/logo192.png" style="height: 50px;">';
  element.onclick = () => {
    chrome.runtime.sendMessage({
      type: "OPEN_VIDEO",
      url: video.src,
